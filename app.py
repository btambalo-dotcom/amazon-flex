...
class Shift(db.Model):
    ...
    manual_hours = db.Column(db.Float, nullable=True)
...
class Trip(db.Model):
    ...
    block_pay = db.Column(db.Float, nullable=False, default=0.0)
    tips = db.Column(db.Float, nullable=False, default=0.0)
    fuel_cost = db.Column(db.Float, nullable=True)
    fuel_volume_gal = db.Column(db.Float, nullable=True)
    ...
    def mpg(self):
        miles = self.miles()
        if self.fuel_volume_gal and self.fuel_volume_gal > 0:
            return miles / self.fuel_volume_gal
        return None

    def cost_per_mile(self):
        miles = self.miles()
        if miles > 0 and self.fuel_cost is not None:
            return self.fuel_cost / miles
        return None
...
def register_routes(app):
    ...
    def new_trip(shift_id):
        ...
        if request.method == 'POST':
            ...
            fuel_volume_gal = parse_float(request.form.get('fuel_volume_gal') or '')
            ...
            t = Trip(
                ...
                tips=tips,
                fuel_cost=fuel_cost,
                fuel_volume_gal=fuel_volume_gal,
    ...
    def edit_trip(trip_id):
        ...
        if request.method == 'POST':
            ...
            t.fuel_volume_gal = parse_float(request.form.get('fuel_volume_gal') or '')
            odo_start = request.form.get('odometer_start') or None
            odo_end = request.form.get('odometer_end') or None
    ...
    def reports_csv():
        ...
        def row(s: Shift):
            ...
            return [s.work_date.isoformat(), f"{hours:.2f}", len(s.trips), f"{earnings:.2f}", f"{fuel:.2f}", f"{miles:.1f}", f"{net:.2f}", f"{per_hour:.2f}", f"{per_mile:.2f}"]
        import csv, io
        output = io.StringIO()
        w = csv.writer(output)
        w.writerow(['Date','Hours','Trips','Earnings','Fuel','Miles','Net','$/hour','$/mile'])
        for s in shifts:
            w.writerow(row(s))
        output.seek(0)
        return Response(output.read(), mimetype='text/csv', headers={'Content-Disposition': 'attachment; filename=relatorio_flex.csv'})
